...
$(document).ready(function () {
    ...
    var details = JSON.parse(detalles);
    $.each(details, function(j,detalle){
        var item = {
            ...detalle,
            i,
        }
    ...
    // console.log("{{ $cotizacion->observacion }}");
    $('#observacion').val(cotizacion_observacion);
    // $('#observacion').val("{{ $cotizacion->observacion }}");
    resizingObs();
...
function validarItemsDiferentes(datoItem) {
    ...
    if (items.length != 0) {
        ...
        const resp = items.filter(
            item => ( item.id_servicio == datoItem.id_servicio ) 
        );
...
function addPrecioTabla(id) {
    ...
    if(mensaje != '') {
        ...
        $('#tuplas_edit_detalles').append('\
            <tr  id="row'+i+'">\
                <td class="td-text" style="width: 30px" > '+i+'</td>\
                ...
                <td class="center-align"> <div class="custom-select">\
                    <select class="browser-default center-align td-text" id="i_precio'+i+'" name="i_precio[]"  >\
                        <option  value="'+precio+'" >'+precio+'</option>\
                    </select>\
                </div></td>\
                <td class="center-align"> <div class="custom-select">\
                    <select class="browser-default center-align td-text" id="i_cantidad'+i+'" name="i_cantidad[]"  >\
                ...
                <td class="center-align"> <div class="custom-select">\
                    <select class="browser-default center-align td-text" id="i_subtotal'+i+'" name="i_subtotal[]"   >\
...
$('#id_grado').change(function () {
    ...
    datos.push({id_servicio: id_servicio, id_vehiculo : id_vehiculo, id_grado: id_grado});
...
function eliminarItem(id, key) {

    console.log(id, key);
    var itemchecked = document.getElementById(`${id}`).checked;
    document.getElementById(`${id}`).checked = !itemchecked;
    console.log(itemchecked);

    var subtotal_i = $(`#i_subtotal${key}`).val();
    console.log('subtotal = ', subtotal_i);

    var subtotal_total = document.getElementById('subtotal_total').value;

    var nuevo_tsubtotal = 0;
    if (itemchecked) {
        //resto el valor a subtotal 
        nuevo_tsubtotal = parseFloat(subtotal_total) - parseFloat(subtotal_i);

    }else{
        //sumo el valor a subtotal
        nuevo_tsubtotal = parseFloat(subtotal_total) + parseFloat(subtotal_i);
    }
    $('#subtotal_total').val(nuevo_tsubtotal);
    ...
    if (total < 0) {
        //TODO: El total = 0 puede pasar si no agrega mas o cuando quiera eliminar todos los items, validar eso
        // alert('El total no puede ser menor a 0!');
        $('#descuento').val(0);
        $('#total').val(nuevo_tsubtotal);
    }else{
